...
The package knows no harness concepts and serves no files: the `/api` HTTP bridge and downlink WebSockets are routes owned by the connection plugin, plugin bundles and the HMR event stream are routes owned by the modules/hmr plugins, and dist serving belongs to the fallback owner. The upgrade handler owns the protocol handshake and connection contents; the webserver only delivers the raw socket and request. `host` accepts any non-empty bind address: `127.0.0.1` (default posture), a specific IP literal or hostname (binds only that interface), or the wildcard literals `0.0.0.0` (every IPv4 interface) and `::` (every IPv6 interface), the deliberate network-exposure posture the CLI refuses. The browser-trust fence decides which authorities may drive the API; this schema only picks where the socket listens. This server serves browsers only; Electron loads dist over `file://` and carries fetch over an IPC bridge. This package never prints; the URL line belongs to the shell.
