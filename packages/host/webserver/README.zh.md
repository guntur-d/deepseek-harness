...
该包不了解任何 harness 概念，也不提供任何文件服务：`/api` HTTP 桥接与下行 WebSocket 是 connection 插件的 route，插件 bundle 与 HMR（热模块替换）事件流是 modules／hmr 插件的 route，dist 服务则属于 fallback 持有者。upgrade handler 拥有协议握手与连接内容；webserver 只交付原始 socket 与 request。`host` 接受任意非空绑定地址：`127.0.0.1`（默认安全姿态）、具体的 IP 字面量或主机名（只绑定该网卡），或通配字面量 `0.0.0.0`（所有 IPv4 网卡）与 `::`（所有 IPv6 网卡），即 CLI 拒绝的刻意向网络开放姿态。由哪个 authority 可以驱动 API 由浏览器信任围栏决定；本 schema 只负责选择 socket 监听位置。该服务器只服务浏览器；Electron 通过 `file://` 加载 dist，并经 IPC 桥接承载 fetch。该包从不打印内容；URL 行属于 shell。
