...
import { randomUuid } from '../api/random-uuid.ts'
...
export abstract class AbstractApiClient implements IApiClient {
  ...
  protected mintRpcId(): RpcId {
    // randomUuid uses crypto.getRandomValues, exposed on insecure origins (plain
    // HTTP LAN access) where crypto.randomUUID is undefined.
