...
| `writeBytes(target, data, expected?, signal?)` | Atomic create/replace of raw bytes — the mirror of `readBytes`, with the same guard contract as `writeText` and the same atomic publication, but no encoding or text-diff basis (the outcome carries no `before`/`after`). |
...
- **Text-only mutations by contract** — text reads and `writeText`/`editText` reject binary/non-UTF-8 content with `FS_NOT_TEXT`; `readBytes`/`writeBytes` are the raw-byte primitives, and model-facing binary tools remain a deliberate deferral of [the tool-schemas Agent Note](../../../.agents/notes/implemented/feature/2026-06-17-filesystem-tool-schemas.md).
