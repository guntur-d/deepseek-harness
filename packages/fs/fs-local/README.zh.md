...
- **`writeBytes`**：`writeText` 的原始字节对应物——相同的原子发布、mode 保留、Windows DACL 处理与可选的 `expected` 防护语义，但没有 UTF-8 编码，也没有上下文 diff 基础（结果只携带 `operation`/`version`）。大小上限由调用方掌握——seam 在没有消费者上限的情况下绝不缓冲。
