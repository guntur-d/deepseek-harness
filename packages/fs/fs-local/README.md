...
- **`writeBytes`** — raw-byte counterpart of `writeText` with the identical atomic publication, mode preservation, Windows DACL handling, and optional `expected` guard semantics, but no UTF-8 encoding and no contextual diff basis (the outcome carries only `operation`/`version`). The caller owns the size bound — the seam never buffers without the consumer's ceiling.
