...
`turn/*`, `step/*`, `user/message`, `assistant/*`, and `tool/*` are durable session events; the rest are live extension points across three domains. `agent/pre-step`, `agent/request`, `llm/stream`, and the three `tools/*` events are waterfalls, whose listeners must call `next()` to delegate; `agent/turn-stopping` is serial and has no `next()`. A step whose tool calls all error repeats; past `maxConsecutiveToolFailures` consecutive all-error steps the loop ends the turn with a logged, model-visible notice instead of driving without bound.
