...
按工作区隔离的记忆服务与其模型可见消费方共享的类型。存储与注入决策见 [持久记忆 Agent Note](../../.agents/notes/implemented/feature/2026-08-14-persistent-cross-session-memory.md)；本页记录来自 [`packages/memory/memory/src/types.ts`](../../packages/memory/memory/src/types.ts) 的精确字段与服务表层。

## 条目

`MemoryId` 是 [品牌化 id](core.md#branded-ids)，由服务在保存时铸造。一条条目是一个持久事实，作用域为保存它的会话所在的工作区。
...
`ctx.memory` 把每个动词都限定在调用者的存活 `Agent` 会话上：`save(agent, input)` 记住一个事实并记录 `memory/write`；`list(agent, { limit? })` 与 `search(agent, query)` 同步读取、最新在前；`forget(agent, id)` 删除一条条目并记录 `memory/forget`。读取从不触碰模型；模型可见表层（`memory_*` 工具与 `app:memory` 提示段落）由 [`dsh-memory-tool`](../../packages/memory/memory-tool/README.md) 拥有。
